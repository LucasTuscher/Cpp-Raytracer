 ...
 * CylinderTests.cpp
 *
 * Unit-Tests für die Cylinder Klasse
 *
 * Tests basierend auf dem Universitätsskript Kapitel 8.4
 */
...
TEST(CylinderTest, RayStrikesCylinder) {
    ...
    for (const auto& c : cases) {
        ...
        EXPECT_EQ(xs.count(), static_cast<size_t>(c.expectedCount));
...
TEST(CylinderTest, NormalVectorOnCylinder) {
    struct NormalCase {
        Point point;
        Vector normal;
    };
...
TEST(CylinderTest, DefaultMinimumAndMaximum) {
    Cylinder cyl;
    EXPECT_TRUE(std::isinf(cyl.minimum));
    EXPECT_TRUE(std::isinf(cyl.maximum));
    EXPECT_LT(cyl.minimum, 0.0);
    EXPECT_GT(cyl.maximum, 0.0);
}
...
TEST(CylinderTest, DefaultClosedValueIsFalse) {
    Cylinder cyl;
    EXPECT_FALSE(cyl.closed);
}

TEST(CylinderTest, IntersectingCapsOfAClosedCylinder) {
    struct Case {
        Point origin;
        Vector direction;
        size_t expectedCount;
